...
IMG ?= ghcr.io/converged-computing/oras-operator
ARMIMG ?= ghcr.io/converged-computing/oras-operator:arm
DEVIMG ?= ghcr.io/converged-computing/oras-operator:test
...
test-deploy: manifests kustomize
	...
	$(KUSTOMIZE) build config/default > examples/dist/oras-operator-dev.yaml

.PHONY: test-deploy-recreate
test-deploy-recreate: test-deploy
	kubectl delete -f examples/dist/oras-operator-dev.yaml || true
...
build-config: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > examples/dist/oras-operator.yaml
...
build-config-arm: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	$(KUSTOMIZE) build config/default > examples/dist/oras-operator-arm.yaml
...
deploy-local: manifests kustomize build
	kubectl delete -f examples/dist/oras-operator-local.yaml || true
	docker build -t ${DEVIMG} .
	cd config/manager && $(KUSTOMIZE) edit set image controller=${DEVIMG}
	$(KUSTOMIZE) build config/default > examples/dist/oras-operator-local.yaml
	sed -i 's/        imagePullPolicy: Always/        imagePullPolicy: Never/' examples/dist/oras-operator-local.yaml
...
.PHONY: clean
clean:
	kubectl delete OrasCache --all --grace-period=0 --force || true
...
#.PHONY: docs-data
#docs-data:
#	go run zack/metrics-gen/main.go docs/_static/data/metrics.json
